...
# NodeA class for storing position, cost and heuristic for each grid encountered
class NodeA:
    # Initialize the class
    def __init__(self, position=None, parent=None):
        self.position = position
    ...
    #This returns the neighbours of the NodeA
    def get_neigbours(self, matrix):
        current_x = self.position[0]
        current_y = self.position[1]
        neighbours = []
    ...
    #This returns all the 8 or 3 neighbours of the NodeA for sensing
...
####################################################################################
@dataclass(order=True)
class PrioritizedItemA:
    priority: float
    item: object = field()

def AstarA(knowledge_grid, start, end, flag=True, heuristic="manhattan"):
    grid_len = len(knowledge_grid)

    # Initialize a priority queue
    pQueue = PriorityQueue()
    pQueue.put(PrioritizedItemA(0.0, start))
    closed_hash = {}    
    counter = 0
    ...
    while not pQueue.empty():
        ...
        for n in current.get_neigbours(knowledge_grid):
            ...
            if flag:
                ...
                (x, y) = n.position
                if knowledge_grid[x][y] != 1:
                    pQueue.put(PrioritizedItemA(float(n.f), n))
...
def implementA(matrix, knowledge, path):
    #follow the path provided by A-star and update the knowledge according to the actual grid
    # print(path)
...
def agent_3(matrix, knowledge, start, end, heuristic="manhattan"):
    #repeat A-star from the last node where the next node encountered is a block
    while True:
        res = AstarA(knowledge, start, end, heuristic)
        path = res[0]
        if path:
            last = implementA(matrix, knowledge, path)
            last_Node = NodeA(last)
            if path[len(path)-1] == last:
                break
            start = last_Node
    ...
    for i in range(1,p0, 4):
        for j in range(40):
            ...
            if res != None:
                ...
                start = NodeA()
                start.position = (0, 0)
                shortest_path = AstarA(knowledge, start, goal, False, "manhattan")[0]
            ...
            # print("Agent2 blocks encountered", blocks_encountered)
            # print("Agent 2 trajectory length", total_trajectory_length)
            result["blocks_for_A2"].append(blocks_encountered)
            result["trajectory_length_A2"].append(total_trajectory_length)
            result["shortest_path_A2"].append(path_len)
